...
1. **Exploratory Data Analysis (EDA):** Performed statistical profiling and visualized fraud patterns across transaction types (TRANSFER, CASH_OUT).
2. **Feature Engineering:** Handled data cleaning, log transformations for transaction amounts, and feature encoding.
3. **Model Training:** Developed a classification pipeline to automate preprocessing and model fitting.
4. **Evaluation:** Validated the model using Confusion Matrix and Classification Reports (Precision, Recall, F1-Score).
5. **Deployment:** Exported the pipeline using Joblib and integrated it into a Streamlit dashboard.
